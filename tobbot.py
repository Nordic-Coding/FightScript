def main():
    ...
    plt.text(x=img_r.shape[0]*1.25,y=img_l.shape[0]+img_r.shape[0]//2,s=name[:len(name)//2]+(name[::-1])[len(name)//2:])
    plt.text(x=img_r.shape[0]*1.25,y=img_r.shape[0]//2,s=(name[::-1])[:len(name)//2]+name[len(name)//2:])
    plt.axis('off')
